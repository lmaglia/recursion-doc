...
\begin{frame}
	\frametitle{Multiple Recursive to Iterative Transformation}
	We have to \textbf{fold} lines $9$ and $11$. Let's see line $9$ first.\\
	Since $done = true$ and by $gg-fibonacci$ definition  $gg-fibonacci(top(p)_0,true,<0,0>) = <0+0,0> = <0,0>$. \\
	Let's now see line $11$. We will use line number $5$ of $ggg-fibonacci$ definition to \textbf{fold}
	\begin{equation}
		\tiny
		\begin{split}
...
\begin{frame}
	\frametitle{Multiple Recursive to Iterative Transformation}
	Revisar esta parte que hay que agregar el caso n = 1 aparte. En las funciones recursivas y aca
	\begin{block}{Iterative version}
		\begin{algorithmic}[1]
			\tiny
